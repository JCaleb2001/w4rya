...
import { useGetMeQuery, useGetSetupStatusQuery, useLoginMutation } from "../api";
...
export function Login() {
  ...
  // A fresh install has no accounts at all, so no password can work here.
  // Send people to the wizard rather than let them lock themselves out
  // guessing at credentials that don't exist yet.
  const { data: setup } = useGetSetupStatusQuery();
  if (me?.user) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (status === 409 && err?.data?.needs_setup) {
        // The install lost its users.yaml between page load and submit.
      ...
      if (status === 401) setErrorMsg("invalid credentials");
        ...
        const wait = err?.data?.retry_after_sec;
        setErrorMsg(
          wait ? `too many attempts; retry in ${wait}s` : "too many attempts; try again later"
        );
